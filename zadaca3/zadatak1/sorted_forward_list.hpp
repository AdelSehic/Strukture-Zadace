...
list<T>::list(const list<T>& other) : sentinel{new node{nullptr}} {
    sentinel -> next = back_ = sentinel;
    auto it = other.begin();
    while(it != other.end()) back_ = back_->next = new node(back_->next, *(it++));
    size_ = other.size_;
}
...
list<T>::list(const std::initializer_list<T>& elementi) : sentinel{new node{nullptr}} {
    sentinel -> next = back_ = sentinel;
    T niz[elementi.size()];
    std::copy(elementi.begin(), elementi.end(), niz);
    std::sort(niz, niz+elementi.size());
    for (size_t i = 0; i < elementi.size(); ++i){
        back_ = back_->next = new node(back_->next, niz[i]);
    }
    size_ = elementi.size();
}
...
template <typename T>
list<T>& list<T>::push(const T& element){
    node* it = sentinel;
    while(true){
        if( element < it -> next -> data || it->next == sentinel ){
            it -> next = new node{it -> next, element};
            if( it -> next -> next == sentinel ) back_ = it->next;
            ++size_;
            return *this;
        }
        else it = it -> next;
...
void list<T>::insert(iterator position, T element){
    ...
    // if(position -> next == sentinel) back_ = position.p_;
    ++size_;
}
